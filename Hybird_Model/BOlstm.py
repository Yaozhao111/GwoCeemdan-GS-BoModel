...
class BayesianOptimizedLSTM():
    ...
    def create_model(self, lstm_units, dense_units, dropout_rate):
        model = Sequential()
        # LSTM model
    ...
    def train_model(self, lstm_units, dense_units, dropout_rate):
        lstm_units = int(lstm_units)
        dense_units = int(dense_units)
        # 创建模型
        model = self.create_model(lstm_units=lstm_units,dense_units=dense_units, dropout_rate=dropout_rate)
        # 早停法
...
if __name__ == "__main__":
    ...
    # 2. 初始化模型
    model = BayesianOptimizedLSTM(n_steps_in=12, batch_size=64, learning_rate=0.001)
    # 3. 数据预处理
    x_train, x_val, x_test, y_train, y_val, y_test = model.prepare_data(arr=data)
    # 4. 执行贝叶斯优化
    print("开始贝叶斯优化...")
    model.bayesian_optimization(x_train, y_train, x_val, y_val, init_points=5, n_iter=20)
    # 5. 使用最佳参数训练最终模型
    print("\n训练最终模型...")
